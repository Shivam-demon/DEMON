...
from Demon import OWNER_ID, SUPPORT_CHAT
...
async def lego(event):
 ...
 if event.sender_id != OWNER_ID and not quew:
  ...
  return
 pesan = await event.reply('**ᴄʀᴇᴀᴛɪɴɢ ᴀ ᴘɪᴇᴄᴇ. ᴘʟᴇᴀsᴇ ᴡᴀɪᴛ!!!**')
 try:
    ...
    fillcolor = "black"
    shadowcolor = "blue"
    fnt = glob.glob("./Demon/resources/fonts/*")
    ...
    fname = "logo.png"
    img.save(fname, "png")
    await telethn.send_file(event.chat_id, file=fname, caption = f"**Made by [ᴅᴇᴍᴏɴ](t.me/im_demon_bot**")         
    await pesan.delete()
    if os.path.exists(fname):
            os.remove(fname)
 except Exception as e:
    await event.reply(f'Error, Report [Sʜɪᴠ](t.me/shivamdemon), {e}')
